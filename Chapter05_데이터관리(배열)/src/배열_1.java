 ...
 * 
 * 	배열의 필요성
 * 	프로그램 : 데이터 여러개를 한 개의 이름으로 관리 => 배열/클래스
...
public class 배열_1 {
	...
	public static void main(String[] args) {
		 ...
		 * 	데이터를 모아서 관리하는 기법: 배열
		 * 
		 * 	=> 같은 데이터형을 여러개 모아서 관리를 한다
		 * 		이름을 한 개를 이용해서 관리할 수 있다
		 * 		=> 장점
		 * 			변수 여러개를 1개의 이름으로 제어 => 반복문 수행하기 용이
		 * 			연속적으로 메모리에 저장 (인덱스 번호 사용 - 0부터 시작)
		 * 		=> 단점
		 * 			고정적이다 => 추가, 삭제하기 어렵다 ==> 보완:가변형(컬렉션)
		 * 					(추가: 큰 배열 생성) (삭제: 작은 배열 생성)
		 * 			같은 데이터형만 모을 수 있다 ==> 보완:클래스
		 * 	
		 * 		변수/배열/클래스 => RAM에 저장
		 * 						프로그램 종료시 사라진다 (파일, 오라클 - CRUD(읽기,쓰기,수정,삭제))
		 * 		
		 * 		배열의 실제값은 heap에 저장되어있고 stack에는 주소값이 저장된다
		 */
