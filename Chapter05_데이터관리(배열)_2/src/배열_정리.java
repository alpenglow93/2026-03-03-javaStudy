 ...
 * 		1) 기능별 나눠서 저장
 * 		2) 데이터형: 같은 데이터형만 저장
 * 					( 작은 데이터형도 저장이 가능 ( int[] arr = {'a', 'b', 'c', 'd'...}; ) -> 형변환
 * 		3) 선언
 * 			데이터형[] 배열명; (권장사항) // 데이터형 배열명[];
 * 		4) 초기값
 ...
 * 		5) 값 변경
 * 			전체변경 for(int i=0; i<arr.length; i++)
 * 			한개변경 arr[0]=10;
 * 		6) 출력
 * 			for-each
 * 				for(변수선언:배열명) => 실제 배열에 저장된 값을 가져옴
 * 		7) 배열 참조
 * 			얕은 복사 : 별칭 => 참조 배열을 동시에 사용이 가능
 * 			깊은 복사 : 배열을 통째로 복사하여 새로운 메모리에 저장
 ...
 * 			int[] arr;
 * 
 * 		String : 문자열을 저장하는 데이터형 + 클래스 ( char[] )
 * 			인덱스번호는 0번부터 시작
 * 			length는 1개가 많다
 * 			배열의 크기 => 배열명.length
 * 			한번 선언하면 => 고정 ( 크기를 키우려면 새로운 배열을 생성해야함)
 * 								크기를 작게 하는것도
 * 			
 *
 * 
	...
	public static void main(String[] args) {
		...
		while(true)
		{
			...
			else if(menu==3)
			{
				...
				{
					if(data.contains(fd))	// contains(): 포함된것을 가져와라
					{
			...
			else if(menu==4)
			{
				...
				{
					if(movie.actor[i].contains(fd))	// contains(): 포함된것을 가져와라
					{
			...
			else if(menu==5)
			{
				...
				{
					if(movie.genre[i].contains(fd))	// contains(): 포함된것을 가져와라
					{
