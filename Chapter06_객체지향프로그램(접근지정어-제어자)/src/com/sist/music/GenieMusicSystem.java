 ...
 * 				=> 패키지에 있는 클래스 여러개를 사용할때
 * 			import 패키지.클래스명 : 지정된 클래스 1개 가지고 온다
 * 	
 * 	3. 접근지정어
 * 			
 * 		private : 데이터 은닉화
 * 				=> 변수 (노출이 안되는 데이터가 있는 경우)
 * 				=> 자신의 클래스에서만 사용이 가능
 * 				=> 제한 허용
 * 					getter/setter : 캡슐화
 * 		default : 같은 패키지까지 접근
 * 				window : Buton, TextField
 * 		protected : 같은 패키지까지 접근, 상속 받는 경우 다른 패키지에 접근이 가능
 * 				사용 빈도가 거의 없다
 * 		public : 모든 클래스에 접근이 가능 => 공개
 * 
 * 		1) 인스턴스 변수 : private (다른걸 사용할수도 있음)
 * 			=> 데이터 노출(X)
 * 			=> 웹에서는 노출이 되면 안 됨 (POST)
 * 				GET으로 전송하면 값이 노출됨
 * 		2) 클래스 : public, default (이 두 가지만 사용 가능)
	...
	static	// 자동으로 호출 => 상속 예외
	{
		try
		{
			...
			for(int i = 0; i < title.size(); i++)
			{
				...
				{
					state = temp.replaceAll("[^가-힣]", "");	// 한글을 제외하고 지워라
					id = temp.replaceAll("[^0-9]", "");	// 숫자를 제외하고 지워라
				}
				m.setState(state);
