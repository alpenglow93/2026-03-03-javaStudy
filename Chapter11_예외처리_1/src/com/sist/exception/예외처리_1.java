 ...
 * 			---------------------------------
 * 			|								|
 * 		IOException						UnCheckException
 * 			입출력 = 메모리, 네트워크, 파일
 * 					| 표준
 * 					  System.out
 * 				  	  System.in
 * 		ClassNotFoundException
 * 			리플렉션 : 클래스 정보를 읽어서 메모리 할당 / 메소드 호출 / 변수값 지정
 * 					new 없이 메모리 할당이 가능
 * 		MalformedURLException : URL주소
 * 		InterrupedException : Thread => 충돌
 * 		=> Thread.sleep()
 * 		SQLException
 * 		------------------------ 무조건 예외처리를 한다 (CheckException)
 * 
 ...
 * 			// => 예상 : 에러 발생에 대한
 * 
 * 			=> 사용자로부터 문자열 정수를 받아서 // 문자열 정수 : NumberFormat
 * 				배열에 저장		// 배열 : ArrayIndexOutOfBoundsException
 * 				나누기한 값을 출력	// 나누기한 값을 출력 : ArithmaticException
 * 		} catch(예외처리 클래스)
 * 		{
 * 		} catch(NumberFormat)
 * 		{
