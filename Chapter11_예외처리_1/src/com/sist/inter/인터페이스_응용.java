...
class MoleGame implements Game
{
	...
	public void start() {
		...
		for(int i = 1; i <= 5; i++)
		{
			...
			long endTime = System.currentTimeMillis(); // millisec : 1/1000초
			
			// 걸린 시간
			long time = endTime-startTime;
			
			if(time >2000)	// 1000이 1초
			{
 ...
 * 	A a = new A();
 * 	B b = new B();
 * 	A c = new B();	// 상위 클래스로 하위 클래스 호출 (동적 바인딩)
 * 	c.a => 10	=> 변수는 A클래스 변수 
 * 	c.display() => B클래스의 메소드 호출
 * 	
 * 	
 * 
 */
...
//		// 객체 비교 instanceof (크거나 같으면 true)
//		if(mole instanceof Game)	// mole이 Game에 포함됐는가?
//		{
...
//		if(sb instanceof str)	// 연관이 없는 객체를 비교하면 에러 발생
//		{
//			
//		}
		/*
