 ...
 * 		=> @functionalInterface
 * 		=> jdk 1.8에서 => 오라클에서 인수 => 개발자 C언어 => C언어에서 필요한 부분 도입
 * 		=> 라이브러리 활용이 많다
 * 			조건 검색 (if) : filter
 * 			화면에 출력 forEach
 * 			=> JWT : 스프링 보안 => 람다
 ...
 * 		forEach() : 화면 출력
 * 					forEach(System.out::pritln) // 메소드 참조
 * 		==> 데이터 읽기 : 파일 / 데이터베이스
 * 					
 * 		~예외처리 => 기본문법	
...
public class Lambda_3 {
	...
	public static void main(String[] args) {
		...
		// 매개변수가 없는 상태
		//() -> System.out.println("Hello");	// 인터페이스가 정의되지 않고 단독으로 사용은 안 됨
...
//				}
			...
			// VIP\r\n : 이렇게 저장되어있어 equals()로는 출력이 안 되고 contains() 사용
			case 2 -> {
//				for(Customer c:cm.cList) 
//				{
...
//				}
				...
				cm.cList.stream()
						...
						.forEach(c->System.out.print(c.getName()+"."+c.getGrade()));	// for
			}
			// => for문, if문 => 제어문을 쉽게 처리
			// 반복 처리 => stream()을 이용해서 처리
			case 3 -> {
				cm.cList.stream()
						.filter(c->c.getLoc().equals("서울"))
