...
public class stream_3 {
	...
	public static void main(String[] args) {
		...
		List<User> adults = users.stream()
								 .filter(u->u.getAge()>=18)	// 조건 만족 데이터를
								 .collect(Collectors.toList());	// 리스트에 담는다
		// List에 조건이 맞는 회원만 담아준다 => toList()
		// 브라우저, 윈도우로 전송
		adults.stream()
		...
		List<Order> kims = orders.stream()
								...
								.collect(Collectors.toList());
		kims.stream().forEach(k->System.out.println(k.getUserName()+", "+k.getPrice()));
