...
public class CustomerList extends JPanel implements ActionListener {
	...
	public CustomerList()
	{
		...
		String[][] row = new String[0][5];	// 한 줄에 5개 출력
		...
		model = new DefaultTableModel(row, col) {
			...
			public boolean isCellEditable(int row, int column) {
				...
				return false;	// 편집방지 (더블클릭하면 편집되는것을 방지)
			}			
		};
		...
		table.getTableHeader().setReorderingAllowed(false);	// 상단 라벨 셀 옮기는것 방지
	...
	{
		// 테이블 지우기
		for(int i = model.getRowCount()-1; i>=0; i--)	// 0번부터 지우면 안 지워진대...
		{
			model.removeRow(i);
		}
