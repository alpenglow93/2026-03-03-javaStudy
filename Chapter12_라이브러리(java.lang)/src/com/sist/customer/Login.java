 ...
 * 		= JDialog => 상단 메뉴 팝업 : JFrame 위에 다른 윈도우
 * 	
 * 	컴포넌트
 * 		= 버튼 종류
 * 			JButton
 * 			JCheckBox , JRadioButton (토글버튼)
 * 			JMenu
 * 		= 입력 종류
...
public class Login extends JFrame {
	...
	{
		// 레이아웃 => 화면 배치
		setLayout(null);	// null값 대입 : 사용자 정의 => 직접 배치
		la1 = new JLabel("아이디");
		la2 = new JLabel("비밀번호");
		...
		JPanel p = new JPanel();	// 패널 위에 버튼을 올려서 가운데 배치
		p.add(b1); p.add(b2);
		p.setBounds(10, 90, 235, 35);
		add(p);
		
		setBounds(850, 400, 265, 175);
		setVisible(true);
	}
