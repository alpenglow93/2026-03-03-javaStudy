 ...
 * 	관련된 데이터를 모아서 관리 => 클래스
 * 		데이터형이 다른 경우
 * 		사용자 정의 데이터형 => 일반 변수와 동일 (형변환 / 대입 가능)
 * 
 * 	1. 정적 변수
 ...
 * 
 * 		reuturn c;
 * 		// c는 지역변수이기 때문에 {}괄호가 끝나면 해제된다.
 *		// 그렇기 때문에 해제 되기 전 값을 넘겨주기 위해 return 사용 
 * 	}
 * 
 * 	=> Heap - 실제 데이터가 저장
 ...
 * 			- 객체는 자신의 영역만 접근이 가능
 * 
 * 	=> MethodArea - 모든 객체가 데이터를 공유할 수 있게 만든다
 * 					- 단 한개의 메모리 생성되는 공간
 * 					- 객체와 독립적인 생명주기
 * 						- 객체가 소멸돼도 메모리를 유지한다
 * 					- 공통 데이터 관리 영역
 ...
 * 					인스턴스 변수				정적 변수
 * 	------------------------------------------------------------
 * 		선언방식		클래스 블록 선언				클래스 블록 선언 = > static
 * 
 * 		저장시점		new 연산자 이용				컴파일시에 자동 저장
 * 					=> 메모리 공간 할당
 * 
 ...
 * 		생명주기		객체가 메모리에 있는 동안		프로그램 종료시까지
 * 
 * 	static => 로그 / 회사명
 * 	나머지는 인스턴스
 * 
