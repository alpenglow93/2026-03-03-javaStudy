...
public class Collection_10 {
	 ...
	 * 	메소드 정리
	 * 	1. 데이터 추가 / 수정
	 * 		put(key,value) => key가 같으면 덮어쓴다 (수정)
	 * 	2. 값 읽기
	 * 		get(key) => key에 해당되는 값을 가져온다
	 * 	3. 삭제
	 * 		remove(key) => Set:remove(실제 데이터)
	 * 					List:remove(int index)
	 * 	4. 키 존재 확인
	 * 		boolean containsKey(key) => if문에서 사용
	 * 	5. 값 존재 확인
	...
	public static void main(String[] args) {
		...
		// 4. 키 존재 (키가 없으면 null값 리턴)
		// boolean containskey(키)
		if(map.containsKey("바나나"))
		{
			System.out.println("바나나가 존재합니다");
		}
		// 5. 값 수정
		map.put("사과", 1000);	// 키가 같으면 덮어쓴다 (키는 중복할 수 없다)
		System.out.println("전체 데이터: " + map);
		// 순서가 없다 (Set)
		// 6. 삭제
		map.remove("포도");
